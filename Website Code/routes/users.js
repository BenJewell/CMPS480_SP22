...
router.post('/login', validate({body: LoginSchema}), function (req, res, next) {
  ...
  // I don't think we need this part anymore, if so, we can put the line above back in.
  
  query("sha1(concat(now(), is, name)) as key from Users where email_address = ? and password = SHA1(?)", [
    req.body.email,
    req.body.password
  ], data => {
    if (!data || !data.length) {
    ...
    query("update Users set session_key = ? where id = ?", [data[0].key, data[0].id], () => {
    });
